...
public:
    ...
    int splitFile(string fileName, int subSize) { 
        ...
        while (!endOfFile) { //заполение вектора данными 
            ...
            int j; 
            for (j = 0; j < subSize; ++j) { 
                Sleep(1000*shift_tape_one_position); //задержка на перемещение по файлу
                if (!inputFile.eof()) { 
                    Sleep(1000*delay_in_reading); // задержка на считывание
                    inputFile >> subData[j]; 
                } else { 
                    endOfFile = true; 
                    break; 
            ...
            for (int k = 0; k < j; ++k) { // заполнение буфера отсортированными числами 
                Sleep(1000*shift_tape_one_position); //задержка на перемещение по файлу
                Sleep(1000*write_delay); //задержка на запись
